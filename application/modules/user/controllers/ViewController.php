<?php

class User_ViewController extends Zend_Controller_Action
{

    public function preDispatch()
    {
        if (!Zend_Auth::getInstance()->hasIdentity()) {
            $this->_helper->redirector('index', 'login');
        }
    }

    public function indexAction()
    {
        $user = Zend_Registry::get('user');
        $this->view->user = $user;
        $this->_helper->viewRenderer('profile');
    }

    public function otherAction()
    {
        if (is_numeric($this->getRequest()->getParam('identifier'))) {
            $this->_forward('id');
        }
        else {
            $this->_forward('slug');
        }
    }
    
    public function idAction()
    {
        // action body
        $filters = array(
            'identifier' => 'Digits',
        );
        $validators = array(
            'identifier' => array(
                'presence' => 'required',
                'NotEmpty',
                new EPIC_Validate_Doctrine_RecordExists(array('table' => 'User_Model_User', 'field' => 'id'))
            ),
        );

        $input = new Zend_Filter_Input($filters, $validators, $this->getRequest()->getParams());
        Zend_Debug::dump($input);
        
        if ($input->isValid()) {
            $user = Doctrine_Core::getTable('User_Model_User')->find($input->identifier);
        }
        else {
            $this->_helper->flashMessenger->addMessage('Invalid id specified.');
            $this->_helper->redirector->gotoSimple('index', 'list', 'user');
        }
        
        $this->view->user = $user;
        $this->_helper->viewRenderer('profile');
    }
    
    public function slugAction()
    {
        // action body
        $options = array('filterNamespace' => 'EPIC_Filter');

        $filters = array(
            'identifier' => 'Slug',
        );
        $validators = array(
            'identifier' => array(
                'presence' => 'required',
                'NotEmpty',
                new EPIC_Validate_Doctrine_RecordExists(array('table' => 'User_Model_User', 'field' => 'slug'))
            ),
        );

        $input = new Zend_Filter_Input($filters, $validators, $this->getRequest()->getParams(), $options);
        
        if ($input->isValid()) {
            $user = Doctrine_Core::getTable('User_Model_User')->findOneBySlug($input->identifier);
        }
        else {
            $this->_helper->flashMessenger->addMessage('Invalid username specified.');
            $this->_helper->redirector->gotoSimple('index', 'list', 'user');
        }
        
        $this->view->user = $user;
        $this->_helper->viewRenderer('profile');
    }
}
